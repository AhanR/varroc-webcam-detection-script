...
def display_video(video_source, stop_event, frame_lock, shared_frame, label):
...
def process_frames(stop_event, frame_lock, shared_frame, model, label):
    while not stop_event.is_set():
        with frame_lock:
        ...
        if frame is not None:
            ...
            # Convert annotated frame to ImageTk format and update the label
...
def start_video(video_path, stop_event, frame_lock, shared_frame, video_label, model):
    stop_event.clear()
    display_thread = threading.Thread(target=display_video, args=(video_path, stop_event, frame_lock, shared_frame, video_label))
    process_thread = threading.Thread(target=process_frames, args=(stop_event, frame_lock, shared_frame, model, video_label))
...
def main(video_path):
    ...
    def start():
        ...
        if display_thread is None or not display_thread.is_alive():
            display_thread, process_thread = start_video(video_path, stop_event, frame_lock, shared_frame, video_label, model)
    ...
    def switch_to_webcam():
        nonlocal display_thread, process_thread
        stop()
        display_thread, process_thread = start_video(0, stop_event, frame_lock, shared_frame, video_label, model)
